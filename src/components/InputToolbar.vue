...
function sessionTitle(): string {
  ...
  if (s.label && s.label.length > 1) {
    return s.label.length > 28 ? s.label.slice(0, 26) + '…' : s.label
  }
  const dir = s.projectDirectory
        ...
        <div class="flex items-center gap-2 text-[11px] text-[#666] pointer-events-none">
          <div class="flex items-center gap-1.5 pointer-events-auto">
            <button
              ...
              :title="session.label || (session.projectDirectory?.split(/[\\\\/]/).pop() ?? 'Session')"
              @mousedown.stop
